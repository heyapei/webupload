# 找到了大佬得仓库地址：
https://github.com/zyt1272999061/webuploader_demo

* 我的修改
* 1 添加了数据库的支持 
* 2 但是还有一点是如何保证多人同时传输同一个文件的覆盖问题 我还在想 太菜了
* 2020年5月24日 是不是可以在合并的操作中使用redis做处理 如果是多个人同时上传了相同md5值的文件，第一个请求合并时，写入redis一个值，等待合并完成后删除redis，第二个合并请求进来后，先读redis中有没有值，如果有值则休眠三秒钟，等待进入下一个循环，再次读取redis，如果此时没有值了，先判断该md5值有没有，如果有则使用这个再进入休眠，没有读一下数据库，看看数据库中有没有值，如果已经有了就是用这个值，如果没有就合并 
